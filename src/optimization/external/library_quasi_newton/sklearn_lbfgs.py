"""
Scikit-learn L-BFGS solver wrapper for quasi-Newton optimization comparison.
"""
...
class SklearnLBFGSWrapper(BaseLibraryWrapper):
    """
    Wrapper for scikit-learn L-BFGS solvers.
    
    Provides quasi-Newton optimization functionality using sklearn's lbfgs solver
    available in Ridge regression and other models.
    """
    
    def __init__(self,
                 loss_type: str = 'ridge',  # Default to ridge since L-BFGS works well with it
                 regularization: float = 0.01,
                 max_iterations: int = 100000,
                 convergence_tolerance: float = 1e-3,
                 random_state: Optional[int] = None,
                 # sklearn L-BFGS specific parameters
                 solver: str = 'lbfgs',
                 feature_scaling: bool = True,
                 **kwargs):
        """
        Initialize sklearn L-BFGS wrapper.
        ...
        Args:
            ...
            solver: Solver to use ('lbfgs' for L-BFGS, 'saga' also available)
            feature_scaling: Whether to scale features
            **kwargs: Additional parameters for sklearn model
        """
        if not SKLEARN_AVAILABLE:
            raise ImportError("scikit-learn is required for SklearnLBFGSWrapper")
        ...
        super().__init__(
            ...
            algorithm_name=f'L-BFGS-{solver}',
            loss_type=loss_type,
    ...
    def _create_external_optimizer(self, n_features: int):
        """
        Create sklearn model with L-BFGS solver.
        ...
        if self.loss_type in ['ols', 'ridge']:
            ...
            print("Warning: Sklearn L-BFGS solver doesn't directly support Lasso. Using Ridge approximation.")
            model_params = {
    ...
    def _optimize_external(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        """
        Run sklearn L-BFGS optimization.
        ...
        try:
            ...
            if hasattr(iterations, '__len__'):  # Sometimes n_iter_ is an array
            ...
            self.callback_history['iterations'] = [iterations]
            ...
            print(f"sklearn L-BFGS optimization failed: {str(e)}")
    ...
    def _get_algorithm_specific_results(self) -> Dict[str, Any]:
        """
        Get sklearn L-BFGS specific results.
        ...
        if self.external_optimizer is not None:
            n_iter = getattr(self.external_optimizer, 'n_iter_', None)
            if hasattr(n_iter, '__len__'):  # Sometimes n_iter_ is an array
            ...
            # Add feature scaling information
            if self.scaler is not None:
                sklearn_specific['feature_scaling'] = {
                    'mean_': getattr(self.scaler, 'mean_', None),
                    'scale_': getattr(self.scaler, 'scale_', None),
                    'var_': getattr(self.scaler, 'var_', None)
                }
...
def create_sklearn_lbfgs_optimizer(loss_type: str = 'ridge',
                                  regularization: float = 0.01,
                                  max_iterations: int = 100000,
                                  convergence_tolerance: float = 1e-3,
                                  random_state: Optional[int] = None,
                                  **kwargs) -> SklearnLBFGSWrapper:
    """
    Factory function to create sklearn L-BFGS optimizer.
    ...
    Returns:
        SklearnLBFGSWrapper instance
    """
    return SklearnLBFGSWrapper(
        loss_type=loss_type,
        regularization=regularization,
        max_iterations=max_iterations,
        convergence_tolerance=convergence_tolerance,
