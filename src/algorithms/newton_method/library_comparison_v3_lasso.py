#!/usr/bin/env python3
"""
Library Comparison v3 - Lasso Regression (Approximate)

=== SO SÁNH THƯ VIỆN: LASSO FOCUS ===

HÀM LOSS: Lasso Regression (Xấp xỉ với Smooth L1)
Công thức: L(w) = (1/2n) * Σ(y_i - ŷ_i)² + λ * Σ|w_i|

So sánh giữa:
1. Implementation tự code Lasso (Pure Newton Lasso, Damped Newton Lasso)
2. SciPy optimize với L1 approximation
3. Sklearn Lasso regression (Coordinate Descent)
4. So sánh với các phương pháp khác (ISTA, FISTA)
