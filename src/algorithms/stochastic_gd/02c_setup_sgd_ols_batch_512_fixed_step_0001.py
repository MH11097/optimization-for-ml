...
def main():
    ...
    # Batch Size Optimization: Large mini-batch (batch_size=512) with optimal step_length=0.001
    model = SGDModel(
        learning_rate=0.01,  # Ignored when using fixed step length
        so_epochs=100,
        random_state=42,
        batch_size=512,  # Large mini-batch
        ham_loss='ols',
    ...
    print(f"\nBatch Size Optimization - Large Mini-batch (batch=512), Step Length 0.001 completed!")
