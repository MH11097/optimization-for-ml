#!/usr/bin/env python3
"""
- Ham loss: Ridge = (1/2n) * ||y - Xw||² + (λ/2) * ||w||²
...
def gradient_descent(X, y, learning_rate=0.01, lam=0.01, max_lan_thu=1000, diem_dung=1e-6):


    print("Training Ridge Gradient Descent...")
    ...
    for lan_thu in range(max_lan_thu):
        # Compute Ridge loss and gradient
        loss_value = tinh_loss_ridge(X, y, weights, 0, lam)
        gradient_w = tinh_gradient_ridge(X, y, weights, lam)
        ...
        # Progress update
        if (lan_thu + 1) % 100 == 0:
            print(f"Iteration {lan_thu + 1}: Ridge Loss = {loss_value:.6f}, Gradient norm = {gradient_norm:.6f}")
    ...
    print(f"Final Ridge Loss: {loss_history[-1]:.6f}")
...
def main():
    """Chạy Ridge Gradient Descent với λ=0.01"""
    print("GRADIENT DESCENT - RIDGE SETUP")
    
    # Setup results directory
    results_dir = Path("data/03_algorithms/gradient_descent/ridge_001_gd")
    ...
    in_ket_qua_danh_gia(metrics, training_time, "Ridge Gradient Descent (λ=0.01)")
    
    # Save results.json
    print("   Lưu kết quả vào results.json...")
    results_data = {
        "algorithm": "Ridge Gradient Descent",
        "loss_function": "Ridge (MSE + L2 regularization)",
        "parameters": {
            "learning_rate": 0.01,
        ...
        "regularization_analysis": {
            ...
            "regularization_strength": "Light (0.01)",
            "overfitting_prevention": "Moderate"
        }
    }
    ...
    def ridge_loss_func(X, y, w):
        return tinh_loss_ridge(X, y, w, 0, 0.01)
    
    # 1. Convergence curves
    print("   Vẽ đường hội tụ...")
    ve_duong_hoi_tu(loss_history, gradient_norms, 
                    title="Ridge GD (λ=0.01) - Convergence Analysis",
    ...
    ve_du_doan_vs_thuc_te(y_test, y_pred_test, 
                         title="Ridge GD (λ=0.01) - Predictions vs Actual",
    ...
    ve_duong_dong_muc_optimization(
        loss_function=ridge_loss_func,
        weights_history=sampled_weights,
        X=X_train, y=y_train,
        title="Ridge GD (λ=0.01) - Optimization Path",
