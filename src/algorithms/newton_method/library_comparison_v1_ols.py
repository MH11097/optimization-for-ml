#!/usr/bin/env python3
"""
Library Comparison v1 - OLS (Ordinary Least Squares)

=== SO SÁNH THƯ VIỆN: OLS FOCUS ===

HÀM LOSS: OLS (MSE thuần túy)
Công thức: L(w) = (1/2n) * Σ(y_i - ŷ_i)²

So sánh giữa:
1. Implementation tự code OLS (Pure Newton OLS, Damped Newton OLS)
2. SciPy optimize cho OLS (BFGS, Newton-CG, L-BFGS-B)
3. Sklearn LinearRegression (analytical solution)
4. Analytical solution tự code
