...
def main():
    ...
    # Khởi tạo model với momentum được điều chỉnh
    model = SGDModel(
        learning_rate=0.0001,  # Learning rate thấp hơn với momentum để ổn định
        so_epochs=100,
        random_state=42,
        batch_size=25000,      # Batch size lớn để ổn định
        ham_loss='ols',
        momentum=0.7,          # Momentum vừa phải để tránh bùng nổ
        use_momentum=True
    )
