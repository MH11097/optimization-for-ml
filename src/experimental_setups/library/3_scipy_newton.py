#!/usr/bin/env python3
"""
Library-based experimental setup - Scipy Newton-CG
Algorithm: Newton-CG method using scipy.optimize.minimize
Configuration: Newton-CG for linear regression optimization
Generated for comparison with custom Newton method implementation
"""
import sys
import os
...
def create_plots(X_test, y_test, y_pred, results, experiment_name):
    ...
    plt.title('Predictions vs Actual Values - Scipy Newton-CG')
    ...
    if 'training_history' in results and results['training_history']:
        ...
        plt.ylabel('Loss')
        plt.title('Training Loss - Scipy Newton-CG')
        plt.yscale('log')
        plt.grid(True)
        plt.savefig(results_dir / "convergence_analysis.png", dpi=300, bbox_inches='tight')
        plt.close()

class NewtonCGRegressor:
    """Newton-CG wrapper for linear regression using scipy.optimize.minimize"""
    ...
    def fit(self, X, y):
        """Fit the model using Newton-CG method"""
        # Add intercept term
        ...
        result = minimize(
            ...
            jac=self._gradient,
            hess=self._hessian,
            options={'maxiter': self.maxiter, 'xtol': self.tol},
            callback=callback
        )
...
def main():
    ...
    print(f"Algorithm: Newton-CG method using scipy.optimize.minimize")
    print(f"Configuration: Newton-CG for linear regression")
    print(f"============================================================")
    
    # Load data
    X_train, X_test, y_train, y_test = load_du_lieu()
    
    # Standardize features (recommended for Newton methods)
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_test_scaled = scaler.transform(X_test)
    
    # Create Newton-CG model
    model = NewtonCGRegressor(tol=1e-6, maxiter=1000)
    ...
    print(f"\nScipy Newton-CG completed successfully!")
    print(f"Configuration: Newton-CG with standardized features")
