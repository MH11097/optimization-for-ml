...
- Gradient = X^T(Xw - y) / n
- Learning Rate: 0.5 (ULTRA HIGH - for extremely fast convergence)
- Max Iterations: 200 (expecting very fast convergence)
- Tolerance: 1e-4 (relaxed for speed)

ĐẶC ĐIỂM: Hội tụ cực nhanh nhưng có thể không ổn định, risk cao overshooting
"""
...
def gradient_descent(X, y, learning_rate=0.5, max_lan_thu=200, diem_dung=1e-4):


    print("Training Ultra Fast Gradient Descent...")
    print(f"   Learning rate: {learning_rate} (ULTRA HIGH)")
    print(f"   Max iterations: {max_lan_thu}")
    print(f"   Tolerance: {diem_dung}")
    print("   WARNING: High learning rate - may oscillate or diverge!")
    ...
    for lan_thu in range(max_lan_thu):
        ...
        # Check for oscillation early (if cost increases)
        if lan_thu > 0 and ols_value > OLS_history[-1]:
            oscillation_count += 1
            if oscillation_count >= 3:
                print(f"⚠️ WARNING: Multiple oscillations detected at iteration {lan_thu}")
                print(f"   Consider reducing learning rate from {learning_rate}")
        ...
        if lan_thu > 0 and abs(OLS_history[-1] - OLS_history[-2]) < diem_dung:
            print(f"⚡ ULTRA FAST convergence after {lan_thu + 1} iterations!")
            break
    ...
    print(f"Training time: {training_time:.2f} seconds (ULTRA FAST!)")
    print(f"Final OLS: {OLS_history[-1]:.6f}")
    print(f"Final gradient norm: {gradient_norm:.6f}")
    print(f"Total oscillations: {oscillation_count}")
    
    return weights, OLS_history, gradient_norms, weights_history, training_time, oscillation_count


def main():
    """Chạy Ultra Fast Gradient Descent"""
    print("GRADIENT DESCENT - ULTRA FAST SETUP")
    print("Extremely high learning rate for maximum speed")
    
    # Setup results directory
    results_dir = Path("data/03_algorithms/gradient_descent/ultra_fast_gd")
    ...
    # Train model
    weights, OLS_history, gradient_norms, weights_history, training_time, oscillation_count = gradient_descent(X_train, y_train)
    
    # Đánh giá model
    print(f"\nĐánh giá model trên test set...")
    metrics = danh_gia_mo_hinh(weights, X_test, y_test)
    in_ket_qua_danh_gia(metrics, training_time, "Ultra Fast Gradient Descent")
    
    # Save results.json
    print("   Lưu kết quả vào results.json...")
    results_data = {
        "algorithm": "Ultra Fast Gradient Descent",
        "parameters": {
            "learning_rate": 0.5,
            "max_iterations": 200,
            "tolerance": 1e-4
        ...
        "stability_analysis": {
            "oscillation_rate": oscillation_count / len(OLS_history),
            "speed_vs_stability_tradeoff": "Ultra high speed, moderate stability"
        }
    }
    ...
    ve_duong_hoi_tu(OLS_history, gradient_norms, 
                    title="Ultra Fast GD - Convergence Analysis",
    ...
    ve_du_doan_vs_thuc_te(y_test, y_pred_test, 
                         title="Ultra Fast GD - Predictions vs Actual",
    ...
    ve_duong_dong_muc_optimization(
        ...
        X=X_train, y=y_train,
        title="Ultra Fast GD - Optimization Path",
