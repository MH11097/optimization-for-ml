"""
PyTorch L-BFGS wrapper for quasi-Newton optimization comparison.
This is similar to the Newton version but optimized for quasi-Newton comparison.
"""
...
class PyTorchLBFGSWrapper(BaseLibraryWrapper):
    """
    Wrapper for PyTorch L-BFGS optimizer for quasi-Newton comparison.
    
    Provides quasi-Newton optimization functionality using PyTorch's L-BFGS
    implementation with detailed tracking and configuration options.
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """
        Initialize PyTorch L-BFGS wrapper for quasi-Newton optimization.
        ...
        Args:
            ...
            history_size: History size for L-BFGS (memory length)
        ...
        try:
            ...
            for epoch in range(self.max_iterations):
                ...
                if hasattr(self.external_optimizer, 'state') and len(self.external_optimizer.state) > 0:
                    ...
                    # Check for L-BFGS convergence indicators
    ...
    def _get_algorithm_specific_results(self) -> Dict[str, Any]:
        ...
        pytorch_specific = {
            ...
            'lbfgs_statistics': {
                'lbfgs_steps': self.lbfgs_steps,
                'function_evaluations': self.function_evals,
                'avg_func_evals_per_step': self.function_evals / max(1, self.lbfgs_steps)
            }
        }
                ...
                for param in param_group['params']:
                    if param in self.external_optimizer.state:
                        ...
                        state_info.update({
                            ...
                            's': len(param_state.get('s', [])) if 's' in param_state else 0,  # Gradient differences
                            'y': len(param_state.get('y', [])) if 'y' in param_state else 0   # Parameter differences
                        })
                        break
...
def create_pytorch_lbfgs_optimizer(learning_rate: float = 1.0,
    ...
    """
    Factory function to create PyTorch L-BFGS optimizer for quasi-Newton comparison.
