...
def newton_method(X, y, regularization=1e-8, max_lan_thu=50, diem_dung=1e-10):
    ...
    print("Training Newton Method for OLS...")
...
def main():
    """Chạy Newton Method cho OLS"""
    print("NEWTON METHOD - STANDARD OLS SETUP")
    
    # Setup results directory
    results_dir = Path("data/03_algorithms/newton_method/standard_ols_newton")
    ...
    in_ket_qua_danh_gia(metrics, training_time, "Newton Method - Standard OLS")
    
    # Save results.json
    print("   Lưu kết quả vào results.json...")
    results_data = {
        "algorithm": "Newton Method - Standard OLS",
    ...
    ve_duong_hoi_tu(loss_history, gradient_norms, 
                    title="Newton Method OLS - Convergence Analysis",
    ...
    ve_du_doan_vs_thuc_te(y_test, y_pred_test, 
                         title="Newton Method OLS - Predictions vs Actual",
                         save_path=str(results_dir / "predictions_vs_actual.png"))
    
    # 3. Optimization trajectory
    print("   Vẽ đường đẳng mực optimization...")
    sample_frequency = max(1, len(weights_history) // 50)  # Newton converges fast, sample more
    ...
    ve_duong_dong_muc_optimization(
        ...
        X=X_train, y=y_train,
        title="Newton Method OLS - Optimization Path",
