...
def main():
    ...
    # Batch Size Optimization: Small mini-batch (batch_size=64) with optimal step_length=0.001
    model = SGDModel(
        learning_rate=0.01,  # Ignored when using fixed step length
        so_epochs=100,
        random_state=42,
        batch_size=64,  # Small mini-batch
        ham_loss='ols',
    ...
    print(f"\nBatch Size Optimization - Small Mini-batch (batch=64), Step Length 0.001 completed!")
