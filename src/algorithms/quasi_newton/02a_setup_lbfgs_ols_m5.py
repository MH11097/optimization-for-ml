#!/usr/bin/env python3
"""
Setup script for L-BFGS Quasi-Newton Method - OLS (Memory = 5)
- Memory size: m = 5 (compact memory)
- Max Iterations: 100
- Tolerance: 1e-6
- Armijo c1: 1e-4
- Wolfe c2: 0.9
...
def main():
    """Chạy L-BFGS Quasi-Newton Method cho OLS với memory=5"""
    print("QUASI-NEWTON METHOD - L-BFGS OLS (m=5) SETUP")
    
    # Load data
    X_train, X_test, y_train, y_test = load_du_lieu()
    
    # Khởi tạo model với L-BFGS memory=5
    model = QuasiNewtonModel(
        ham_loss='ols',
        method='lbfgs',      # L-BFGS method
        memory_size=5,       # Compact memory for efficiency
        diem_dung=1e-5,
        armijo_c1=1e-4,
        wolfe_c2=0.9,
        backtrack_rho=0.8,
    ...
    print(f"\nL-BFGS OLS (m=5) training completed!")
    print(f"Memory size: {model.memory_size}")
    print(f"Final memory usage: {results.get('lbfgs_memory_usage', 0)}")
