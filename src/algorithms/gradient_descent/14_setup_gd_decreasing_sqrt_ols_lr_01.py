...
def main():
    ...
    model = GradientDescentModel(
        ...
        so_lan_thu=1000,
        diem_dung=1e-5,
        step_size_method='decreasing_sqrt'  # α / sqrt(iteration + 1)
    )
    ...
    ten_file = get_experiment_name()  # Sẽ là "setup_gd_decreasing_sqrt_ols_lr_01"
    results_dir = model.save_results(ten_file)
    
    # Tạo biểu đồ
    model.plot_results(X_test, y_test, ten_file)
    
    print(f"\nDecreasing Sqrt Step Size training completed!")
