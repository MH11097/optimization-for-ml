...
def newton_method_optimization(X_train, y_train, X_test, y_test, 
    ...
    hessian = compute_hessian_linear_regression(X_train, regularization)
    ...
    for iteration in range(max_iterations + 1):
        ...
        train_mse = compute_mse(y_train, predictions_train)
        ...
        test_mse = compute_mse(y_test, predictions_test)
    ...
    # Final metrics
    final_predictions_train = predict(X_train, weights, bias)
    final_predictions_test = predict(X_test, weights, bias)
    
    final_train_mse = compute_mse(y_train, final_predictions_train)
    final_test_mse = compute_mse(y_test, final_predictions_test)
