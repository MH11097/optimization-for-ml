...
def main():
    ...
    model = GradientDescentModel(
        ham_loss='ols',
        learning_rate=0.1,  # Base learning rate cho Wolfe line search
        diem_dung=1e-5,
        step_size_method='wolfe_conditions',
        backtrack_c1=1e-4,  # Armijo constant (sufficient decrease)
        wolfe_c2=0.9        # Curvature condition parameter
    )
    ...
    ten_file = get_experiment_name()  # Sẽ là "setup_gd_wolfe_conditions_ols_c1_0001_c2_09"
    results_dir = model.save_results(ten_file)
    
    # Tạo biểu đồ
    model.plot_results(X_test, y_test, ten_file)
    
    print(f"\nWolfe Conditions Line Search training completed!")
