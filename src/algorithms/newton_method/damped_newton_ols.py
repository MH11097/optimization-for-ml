...
def backtracking_line_search(X, y, weights, direction, gradient, 
    ...
    """
    Backtracking line search với Armijo condition cho OLS
    """
...
def damped_newton_method(X, y, regularization=1e-8, max_lan_thu=100, diem_dung=1e-8,
                        armijo_c1=1e-4, backtrack_rho=0.8, max_line_search_iter=50):
    print("Training Damped Newton Method v1 for OLS...")
    ...
    # Precompute Hessian (constant for OLS)
    H = tinh_hessian_OLS(X)
    
    # Add regularization for numerical stability
    H_reg = H + regularization * np.eye(n_features)
    condition_number = np.linalg.cond(H_reg)
    ...
    for lan_thu in range(max_lan_thu):
        ...
        try:
            direction = -giai_he_phuong_trinh_tuyen_tinh(H_reg, gradient_w)
            
            # Line search để tìm step size
            step_size, ls_iter = backtracking_line_search(
                X, y, weights, direction, gradient_w, 
...
def main():
    """Chạy Damped Newton Method v1 cho OLS"""
    print("DAMPED NEWTON METHOD V1 - OLS SETUP")
    
    # Setup results directory
    results_dir = Path("data/03_algorithms/newton_method/damped_newton_v1_ols")
    ...
    in_ket_qua_danh_gia(metrics, training_time, "Damped Newton Method v1 - OLS")
    
    # Save results.json
    print("   Lưu kết quả vào results.json...")
    results_data = {
        "algorithm": "Damped Newton Method v1 - OLS", 
        ...
        "numerical_analysis": {
            ...
            "regularization_effect": "Minimal - only for numerical stability"
        }
    }
    ...
    ve_duong_hoi_tu(loss_history, gradient_norms, 
                    title="Damped Newton Method v1 OLS - Convergence Analysis",
    ...
    ve_du_doan_vs_thuc_te(y_test, y_pred_test, 
                         title="Damped Newton Method v1 OLS - Predictions vs Actual",
    ...
    ve_duong_dong_muc_optimization(
        ...
        X=X_train, y=y_train,
        title="Damped Newton Method v1 OLS - Optimization Path",
