...
def main():
    ...
    ten_file = get_experiment_name()  # Sẽ là "setup_gd_exponential_decay_ols_lr_01_gamma_095"
    results_dir = model.save_results(ten_file)
    
    # Tạo biểu đồ
    model.plot_results(X_test, y_test, ten_file)
    
    print(f"\nExponential Decay training completed!")
    print(f"Final step size: {results['step_sizes_history'][-1]:.6f}")
    print(f"Decay gamma: {model.decay_gamma}")
