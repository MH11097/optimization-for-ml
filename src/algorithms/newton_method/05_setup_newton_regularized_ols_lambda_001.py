#!/usr/bin/env python3
"""
Setup script for Regularized Newton Method - OLS
- Hessian regularization: lambda = 0.01 for stability
- Max Iterations: 100
- Tolerance: 1e-8
- Pure Newton step (no line search)
"""
...
def main():
    """Chạy Regularized Newton Method cho OLS"""
    print("NEWTON METHOD - REGULARIZED NEWTON OLS SETUP")
    
    # Load data
    X_train, X_test, y_train, y_test = load_du_lieu()
    
    # Khởi tạo model với Hessian regularization
    model = NewtonModel(
        ham_loss='ols',
    ...
    ten_file = get_experiment_name()  # Sẽ là "setup_newton_regularized_ols_lambda_001"
    results_dir = model.save_results(ten_file)
    
    # Tạo biểu đồ
    model.plot_results(X_test, y_test, ten_file)
    
    print(f"\nRegularized Newton training completed!")
